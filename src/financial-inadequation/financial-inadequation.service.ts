...
export class FinancialInadequationService {
  ...
  async getFinancialInadequationByEpci(epciCode: string) {
    const financialInadequation = await this.prismaService.financialInadequation.findFirstOrThrow({
      where: { epciCode },
    })
    return { data: financialInadequation.nbAllPlus30ParcLocatifPrive }
  }
