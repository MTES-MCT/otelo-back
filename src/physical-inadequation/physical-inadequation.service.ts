...
export class PhysicalInadequationService {
  ...
  async getPhysicalInadequationByEpci(epciCode: string) {
    const physicalInadequation = await this.prismaService.physicalInadequation_Filo.findFirstOrThrow({
      where: { epciCode },
    })

    return { data: physicalInadequation.suroccLourdeLp + physicalInadequation.suroccLourdePo }
  }
